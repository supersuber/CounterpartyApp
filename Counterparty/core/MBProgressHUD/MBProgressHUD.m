...
- (void)layoutSubviews {
    ...
    CGRect indFrame = (indicator != nil ? indicator.bounds : CGRectMake(0, 0, 1, 1));
    ...
    if (nil != self.labelText) {
        ...
        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                   ...
                                   lWidth, lHeight);
        if (lFrame.size.height > 0 && lFrame.size.width > 0) {
            label.frame = lFrame;
        }
        else{
            label.frame = CGRectMake(80, 120, 160, 160);
        }
        ...
        if (mode != MBProgressHUDModeCustomView) {
            ...
            label.font = [UIFont boldSystemFontOfSize:LABELFONTBIGSIZE];
        }
